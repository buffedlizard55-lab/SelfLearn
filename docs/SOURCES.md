...
| `patentsview` | PatentsView API | U.S. Patent and Trademark Office (data platform) | Data published by a government, standards body or international agency. | yes (PATENTSVIEW_API_KEY) | <https://developer.uspto.gov/api-catalog/patentsview> |
...
1. **PatentsView has moved.** PatentsView was folded into the USPTO Open Data Portal,
   and clients are directed to obtain an Open Data Portal key and use the portal's
   endpoints. The register's documentation link now points at
   <https://developer.uspto.gov/api-catalog/patentsview>, but the adapter still calls
   the older `search.patentsview.org` endpoint. Until the adapter is updated, patent
   polling may fail; the failure will be recorded as an `error` irregularity with the
   HTTP status.
